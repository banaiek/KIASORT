...
% Main spike detection function with GPU acceleration
    if nargin < 3
        sample = 0;
    end
    ...
    if isfield(cfg, 'useGPU') && cfg.useGPU && gpuDeviceCount > 0
        useGPU = true;
        bandpass_signal = gpuArray(bandpass_signal);
        rms_bands = gpuArray(rms_bands);
        scale_factor = gpuArray(scale_factor);
    end
    ...
    if sample
        if min(rms_bands(1) ./ rms_bands(2:end)) > badChannel_factor
            ...
            if useGPU, out = gather_output(out); end
            return;
    ...
    mad_Thresh = min_thr * bp_rms_adj;
        ...
        if useGPU, mad_Thresh = gpuArray(mad_Thresh); end
    ...
    % Loc   = detect_peaks(bandpass_signal, -mad_Thresh, 1, -1, 1);
    % Loc_r = detect_peaks(bandpass_signal, mad_Thresh, 1, 1, 1);
...
function out = gather_output(out)
% Gather GPU arrays
    out.spk_idx = gather(out.spk_idx);
    out.spk_Val = gather(out.spk_Val);
    out.spk_ID = gather(out.spk_ID);
    out.scale_factor = gather(out.scale_factor);
    fn = fieldnames(out.rms_values);
    for i = 1:numel(fn)
       out.rms_values.(fn{i}) = gather(out.rms_values.(fn{i}));
    end
end