function kiaSort_explore_sorted_results_gui(cfg, parentPanel, figColor, parentFig)
...
for iCh = 1 : numChannels
    chanLabel{iCh,1} = sprintf('Ch %d',iCh);
end
...
% Update visibility status based on current selection
    ...
    function onMUA()
        ...
        if ~isempty(visibleGroups)
            switch unitIsolation{visibleGroups(1)}
                case 'NA'
                    for i = 1:length(visibleGroups)
                        unitIsolation{visibleGroups(i)} = 'SUA+';
                    end
                case 'SUA+'
                    for i = 1:length(visibleGroups)
                        unitIsolation{visibleGroups(i)} = 'SUA';
                ...
                case 'SUA'
                    for i = 1:length(visibleGroups)
                        unitIsolation{visibleGroups(i)} = 'MUA+';
                    end
                case 'MUA+'
                    for i = 1:length(visibleGroups)
                        unitIsolation{visibleGroups(i)} = 'MUA';
                ...
                case 'MUA'
                    for i = 1:length(visibleGroups)
                        unitIsolation{visibleGroups(i)} = 'NA';
                    end
            end
        end
        refreshLabels();
    ...
    function onSave()
        ...
        function waveforms = extractCuratedWaveforms()
            ...
            for batch = 1:num_batches
                ...
                cols = sortedRes.spike_idx(batch_indices) + (-numSpikeSamples:numSpikeSamples);
                rows = repmat(channel_mapping(sortedRes.channelNum(batch_indices)), 1, waveformLength);

                [data_rows, data_cols] = size(mappedData.Data.data);

                valid_rows = rows <= data_rows & rows > 0;
                valid_cols = cols <= data_cols & cols > 0;
                valid_mask = valid_rows & valid_cols;

                valid_indices = find(valid_mask);
                [batch_row_idx, col_idx] = ind2sub(size(valid_mask), valid_indices);
                row_idx = batch_indices(batch_row_idx);

                linear_indices = sub2ind([data_rows, data_cols], rows(valid_mask), cols(valid_mask));
                waveform_data = zeros(size(valid_mask));
                waveform_data(valid_mask) = mappedData.Data.data(linear_indices);

                for i = 1:length(batch_indices)
                    if any(batch_row_idx == i)
                        waveforms(batch_indices(i), :) = waveform_data(i, :);
                    end
                end
            end
        end
    ...
    function updateRealignSpikes(sVal)
        ...
        for k = selectedGroups'
            shiftedSpikes = sortedRes.unifiedLabels == groupList(k);
            shiftVal = round(sVal * cfg.samplingFrequency/1000);

            BATCH_SIZE = 10000;
            indices = find(shiftedSpikes);
            numBatches = ceil(length(indices) / BATCH_SIZE);

            for batch = 1:numBatches
                startIdx = (batch-1) * BATCH_SIZE + 1;
                endIdx = min(startIdx + BATCH_SIZE - 1, length(indices));
                batchIndices = indices(startIdx:endIdx);

                sortedRes.spike_idx(batchIndices) = sortedRes.spike_idx(batchIndices) - shiftVal;
            end
...
disimlarityScore(15,14)
    ...
    function onSelectSimilarPairs(val)
        ...
        if thisGroup >= 1 && thisGroup <= length(colGroup)
            ...
            if isvalid(groupVisCheckboxes(thisRow))
            ...
            if preprocessed.firingRate(thisCol) > preprocessed.firingRate(thisRow)
                if isvalid(groupRadioButtons(thisCol))
                    groupRadioButtons(thisCol).Value = true;
                end
            else
                if isvalid(groupRadioButtons(thisRow))
...
%% All supporting functions
        ...
        if any(spike_idx) && ~isempty(selected_order)
            for k = selected_order
                ...
                if length(scatterIdx) > 1E4
                    scatterIdx = randsample(scatterIdx, 1E4);
                end
    ...
    function plotWaveforms(panelOption)
        ...
        if isempty(maxSignal)
            maxSignal = max(abs(double(mappedData.Data.data(channel_mapping(chan_wave_inclusion), xDataRange))), [], 'all');
        end
        ...
        % Process each group and plot waveforms
        plotCount = 0;
        minAx=inf;
        maxAx=-inf;
        ...
        max_YAx = -inf;
        min_YAx = inf;
        uniqPlots = length(unique(groupList(selected_order)));
        ...
        if any(spike_idx) && ~isempty(selected_order)
            for k = selected_order
                ...
                chPlot = channelPlot(k, :);
                chPlot = chPlot(ismember(chPlot, yDataRange));
                if ~any(chPlot(ismember(chPlot, yDataRange)))
                    continue
                end
                ...
                for cIdx = 1:numChPlots
                    ...
                    Waveforms(:, cIdx, :) = ampScale * reshape(normIn(rowIdx, all_indices), [numSpikes, 1, numel(spike_Xaxis)]);
                end
                ...
                for cIdx = 1:numChPlots
                    ...
                    plot(plotAxis, [t(round(length(t)/2)) t(round(length(t)/2))], [yNorm_adj(chan)-1 yNorm_adj(chan)+1], 'Color', 1-figColor, 'LineWidth', lineWidth/10, 'LineStyle','-');
                end
                plotCount = plotCount+1;
            end
        end
        ...
        if any(spike_idx) && ~isempty(selected_order)
            for k = selected_order
                ...
                if length(scatterIdx) > 5E4
                    scatterIdx = randsample(scatterIdx, 5E4);
                end
                plot_spike_idx = spike_idx(scatterIdx);
                if ~isempty(plot_spike_idx)
                    c = getGroupColor(groupList(k));
