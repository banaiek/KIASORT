...
for k = 1:3
    ...
    for i=1:numChannels
        ...
        if ~isempty(sampleFeatures{i, 1})
            ...
            clusterStatus_i = sortedSamples{i, 1}.clusteringInfo.clusterSelection.clusterStatus;
            notKeep_i = find(~clust_select_i.keep);

            if ~isfield(clust_select_i, 'transferFoundRate')
                clust_select_i.transferFoundRate = ones(size(clust_select_i.keep));
            end
            ...
            if any(notKeep_i)
                for iKeep = 1:length(notKeep_i)
                    ...
                    status_i = clusterStatus_i(notKeep_i(iKeep));
                    if lbl_i == -1 || status_i < 0
                        continue
                    end
                    clust_spk_i = spk_idx_i(spk_lbl_i == lbl_i);
                    relabeld_ch_i = max_channelIdx_i(notKeep_i(iKeep));

                    if isempty(sampleFeatures{relabeld_ch_i, 1})
                        clust_select_i.clusterStatus(notKeep_i(iKeep)) = -5;
                        clust_select_i.transferFoundRate(notKeep_i(iKeep)) = 0;
                    ...
                    if sum(valid_idx)/length(valid_idx) > .25
                        ...
                        clusterSampleCounts = arrayfun(@(l) sum(transfered_lbl == l), unique_lbl_j);
                        [bestMatch_val,bestMatch_lbl] = max(clusterSampleCounts./length(clust_spk_i));

                        if bestMatch_val < .25
                            clust_select_i.clusterStatus(notKeep_i(iKeep)) = -5;
                            clust_select_i.transferFoundRate(notKeep_i(iKeep)) = bestMatch_val;
                            sortedSamples{i, 1}.clusteringInfo.clusterSelection = clust_select_i;
                            continue
                        end
                        ...
                        if max_channelIdx_j(bestMatch_lbl) == i
                            ...
                            if sum(valid_idx_j_i)/length(valid_idx_j_i) > .25
                                ...
                                clusterSampleCounts_j_i = arrayfun(@(l) sum(transfered_lbl_j_i == l), unique_lbl_i);
                                ...
                                if bestMatch_lbl_ji == notKeep_i(iKeep)
                                    ...
                                    clust_select_i.transferFoundRate(notKeep_i(iKeep)) = 1;
                        ...
                        clust_select_i.clusterStatus(notKeep_i(iKeep)) = -5;
                        clust_select_i.transferFoundRate(notKeep_i(iKeep)) = sum(valid_idx)/length(valid_idx);
                        sortedSamples{i, 1}.clusteringInfo.clusterSelection = clust_select_i;
                    end
